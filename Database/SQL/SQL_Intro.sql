...
SELECT e.EmployeeID, e.FirstName + '.' + e.LastName + '@telerik.com' AS 'Full Email Adress'
FROM Employees e
...
SELECT FirstName + ' ' + LastName AS FullName
FROM Employees WHERE FirstName LIKE 'SA%'

--12. Write a SQL query to find the names of all employees whose last name contains "ei".
SELECT FirstName + ' ' + LastName AS FullName
FROM Employees WHERE FirstName LIKE '%ei%'
...
SELECT e.FirstName + ' ' + e.LastName AS EmployeeName, a.AddressText
FROM Employees e, Addresses a
WHERE e.AddressID = a.AddressID

--20. Write a SQL query to find all employees along with their manager.
SELECT m.LastName + ' manager of ' + e.LastName
FROM Employees e
JOIN Employees m
...
SELECT e.FirstName + ' ' + e.LastName AS FullName,
	m.LastName AS Manager, a.AddressText AS Adress
FROM Employees e
JOIN Employees m
ON (e.ManagerID = m.EmployeeID)
JOIN Addresses a
...
WHERE d.DepartmentID = e.DepartmentID AND d.Name IN ('Sales', 'Finance')
